...
4. 按 `Control + Shift + O`（macOS）或 `Alt + Shift + O`（其他平台）可直接打开侧边栏。
5. 在网页内按 `Control + Shift + H`（macOS）或 `Alt + Shift + H`（其他平台）进入高亮选择模式；此后鼠标选中即高亮，再按一次退出。进入选择模式时侧边栏会自动打开。
6. 在侧边栏点击「设置」配置保存位置和背景色；默认跟随浏览器外观。
