...
2. 选择本项目目录。
3. 点击扩展图标即可打开浏览器原生侧边栏；它由 Chrome 单独分栏展示，不会遮挡或改变网页的 DOM 与布局。
4. 在网页内按 `Control + Shift + H`（macOS）或 `Alt + Shift + H`（其他平台）进入高亮选择模式；此后鼠标选中即高亮，再按一次退出。进入选择模式时侧边栏会自动打开。
5. 在侧边栏点击「设置」配置保存位置和背景色。
